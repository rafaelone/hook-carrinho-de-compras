...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
    ...
    try {
      ...
      if(!existingProduct){
        ...
        newCart = cart.map(product => product.id !== productId ? product : {
          ...product,
          amount: product.amount >= stock.amount ? product.amount  : product.amount + 1
        })
        setCart(newCart)
      }
  ...
  const updateProductAmount = async ({
    ...
    try {
      ...
      if (stock.amount <= 1 || amount > stock.amount) return toast.error('Quantidade solicitada fora de estoque')
